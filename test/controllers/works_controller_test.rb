...
describe WorksController do
  ...
  describe "upvote" do
    it "redirects to the work page if no user is logged in" do
      #ensures that no user is logged in at the moment
      delete logout_path
      assert_nil(session[:user_id])
      #tries to upvote one of the works
      work = Work.first
    ...
    it "redirects to the work page after the user has logged out" do
      #destroys all votes to prevent possible conflicts
      Vote.destroy_all
      #identifies a user and logs them in
      user = users(:dan)
      perform_login(user)
      
      #idenfies a work and upvotes it
      work = works(:another_album)
      
      #checks that upvote was successful 
      post upvote_path(work.id)
      
      expect(Vote.all.count).must_equal 1
      delete logout_path
      
      must_redirect_to root_path
      #differs from original language of test
    end
    
    it "succeeds for a logged-in user and a fresh user-vote pair" do
      #tested as part of previous test with deletion of all votes and the successful increment of votes by 1
    end
    ...
    it "redirects to the work page if the user has already voted for that work" do
      ...
      user = users(:dan)
      perform_login(user)
      #identifies a work to be upvoted
      work = works(:another_album)
      post upvote_path(work.id)
      post upvote_path(work.id)
      #only one vote should be successfully saved. Checked on next line
      expect(Vote.all.count).must_equal 1
