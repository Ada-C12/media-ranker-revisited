...
describe WorksController do
  ...
  describe "Logged in users" do
    ...
    describe "upvote" do
      it "succeeds for a logged-in user and a fresh user-vote pair" do
        perform_login
        starting_votes = existing_work.votes.count

        post upvote_path(existing_work.id)
        must_redirect_to work_path(existing_work.id)
        expect(existing_work.votes.count).must_equal starting_votes + 1
      end
  ...
  describe "Guest users" do
    ...
    it "cannot access update" do
      patch work_path(existing_work.id)
      must_redirect_to root_path
      flash[:status].must_equal :failure
      flash[:result_text].must_equal "You must be logged in to view this section"
    end
    ...
    describe "upvote" do
      it "redirects to the homepage if no user is logged in" do
        ...
        post upvote_path(existing_work.id)
        must_redirect_to root_path
        expect(existing_work.votes.count).must_equal starting_votes
      end
