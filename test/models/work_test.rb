...
describe Work do
  ...
  describe "validations" do
    ...
    it "requires a title" do
      work = Work.new(category: "album", user_id: user.id)
      work.valid?.must_equal false
      work.errors.messages.must_include :title
    end
    
    it "requires a user id" do
      work = Work.new(title: "test", category: "album")
      work.valid?.must_equal false
      work.errors.messages.must_include :user_id
    end
  ...
  describe "top_ten" do
    before do
      ...
      login_user = test_users[0]
      
      # Create media to vote upon
      Work.where(category: "movie").destroy_all
      8.times do |i|
        work = Work.create!(category: "movie", title: "test movie #{i}", user_id: login_user.id)
